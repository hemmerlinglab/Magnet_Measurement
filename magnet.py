...
print('Expecting: {}'.format(d))

plt.subplot(121)
plt.scatter(z1,B1,color='r')
plt.plot(x_fit1,y_fit1,linestyle='-')
plt.xlabel('z (mm)')
plt.ylabel('Magnetic Flux (Gs)')
plt.ylim(0,6200)
plt.xlim(np.min(z1),np.max(z1))
plt.title('Hole Away')

plt.subplot(122)
plt.scatter(z2,B2,color='r')
plt.plot(x_fit2,y_fit2,linestyle='-')
plt.xlabel('z (mm)')
plt.ylim(0,6200)
plt.xlim(np.min(z1),np.max(z1))
